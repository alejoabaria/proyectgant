    ...
    <div class="container mt-5">
        ...
        if ($result_proyectos_alumno->num_rows > 0) {
            while ($row_proyecto = $result_proyectos_alumno->fetch_assoc()) {
                ...
                echo '<a href="tareas.php?id_proyecto=' . urlencode($row_proyecto['id']) . '" class="btn btn-ver-tareas">Ver Tareas</a>';
                echo '<a href="gant.php?id_proyecto=' . urlencode($row_proyecto['id']) . '" class="btn btn-ver-mas">Diagrama Gantt</a>';
                
                // Botón de Editar
                echo '<button class="btn btn-editar" onclick="abrirModalEditar(' . $row_proyecto['id'] . ', \'' . htmlspecialchars($row_proyecto['nombre_proyecto']) . '\', \'' . htmlspecialchars($row_proyecto['descripcion']) . '\', \'' . htmlspecialchars($row_proyecto['fecha_inicio']) . '\', \'' . htmlspecialchars($row_proyecto['fecha_fin']) . '\')">Editar</button>';
                
                // Botón de Eliminar
                echo '<button class="btn btn-eliminar" onclick="eliminarProyecto(' . $row_proyecto['id'] . ')">Eliminar</button>';
